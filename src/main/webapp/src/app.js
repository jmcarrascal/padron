...
angular.module('app', ['ngResource', 'ng.services', 'ngCookies']).
  ...
  $routeProvider.    	
      ...
      when('/busqueda', {
          templateUrl: 'src/views/busqueda.html',
          controller: BusquedaController
          }).
          
      when('/alta', {
          templateUrl: 'src/views/alta.html',
          controller: AltaController
          }).
          
          when('/exportarImportar', {
              templateUrl: 'src/views/exportarImportar.html',
              controller: ExportarImportarController
              }).
          
     when('/padron', {
          templateUrl: 'src/views/padron.html',
          controller: PadronController
          }).
          
       when('/pendienteEnvios', {
          templateUrl: 'src/views/pendienteEnvios.html',
          controller: PendienteEnviosController
          }).
       when('/login', {
...
function getBaseUrl($location) {
	...
	return "http://192.168.0.120:8080/padron";
//	
//	if (protocol != '' && host != '' && port != '') {
//		return protocol + '://' + host + ':' + port + '/webplatform';
//	} else {
//		//return "http://localhost:8080/webplatform";
//
...
function getLoginUrl($location) {
	...
	var url;
	url = "http://192.168.0.120:8080/padron";
//	if (protocol != '' && host != '' && port != '') {
//		url = protocol + '://' + host + ':' + port + '\:' + port + '/webplatform';
//	} else {
//		//url = "http://localhost:8080/webplatform";
//		
//
